...
namespace Geometry_Invasion
{
    internal class Enemy
    {
        ...
        public int type, strength, resistance = 0, direction, speed, team, size, reload = 0, shots = 1, reloadTimer, id = -10, target = -1, homing = 0, targetType = 0, segments = 0;
        ...
        public void ShapeSetup()
        {
            ...
            switch (type)
            {
                case 0: // Player Circle
                    ...
                    break;
                case 1: // Basic Square
                    SetStats(Color.Goldenrod, 1000, 7, 5, 22, new int[] { 0, 2 }, new int[] { 3, 3 }, new int[] { 1, 2 });
                    break;
                case 2: // Fast Triangle
                    SetStats(Color.Lime, 700, 10, 3, 17, new int[] { 3, 9, 10 }, new int[] { 3, 1, 1 }, new int[] { 0 });
                    homing = 1;
                    weight = 0.5;
                    break;
                case 3: // Heavy Pentagon
                    SetStats(Color.Purple, 2000, 4, 7, 27, new int[] { 0, 2, 7 }, new int[] { 6, 4, 2 }, new int[] { 1, 2 });
                    homing = 1;
                    scoreValue = 2;
                    weight = 5;
                    break;
                ...
                case 5: // Splitting Hexagram
                    SetStats(Color.OrangeRed, 1500, 7, 10, 27, new int[] { 2, 1, 12, 0 }, new int[] { 5, 3, 3, 2 }, new int[] { 0 });
                    homing = 2;
                    scoreValue = 1.8;
                    weight = 2;
                    break;
                case 6: // Split Triangle
                    SetStats(Color.OrangeRed, 750, 7, 10, 27, new int[] { 4, 1, 13 }, new int[] { 4, 2, 1 });
                    homing = 2;
                    break;
                case 7: // Small Shooting Pentagram
                    SetStats(Color.Yellow, 700, 4, 2, 14, new int[] { 6, 9, 1 }, new int[] { 2, 2, 2 });
                    reload = 30;
                    weight = 0.1;
                    targetType = 2;
                    break;
                case 8: // Centipede Hexagons
                    SetStats(Color.DarkCyan, 600, 6, 5, 25, new int[] { 0, 4 }, new int[] { 1, 1 });
                    homing = 3;
                    scoreValue = 0.7;
                    break;
                case 9: // Clustered Heptagrams
                    SetStats(Color.BlueViolet, 800, 6, 4, 16, new int[] { 5, 12 }, new int[] { 3, 1 });
                    homing = 2;
                    weight = 0.5;
                    break;
                case 10: // Quad-Shooting Octagram
                    SetStats(Color.Magenta, 800, 4, 6, 26, new int[] { 6, 1, 4, 9 }, new int[] { 6, 4, 2, 1 }, new int[] { 2 });
                    reload = 120;
                    shots = 4;
                    targetType = 2;
                    break;
                case 11: // Charging Hexagon
                    SetStats(Color.DarkRed, 1200, 4, 10, 27, new int[] { 10, 13, 2, 8 }, new int[] { 4, 4, 2, 1 });
                    weight = 3;
                    scoreValue = 1.2;
                    reload = 120;
                    shots = 0;
                ...
                case 12: // Resistant Octagon
                    SetStats(Color.Olive, 400, 8, 2, 20, new int[] { 11, 3, 7 }, new int[] { 4, 3, 1 }, new int[] { 1, 2 });
                    homing = 1;
                    weight = 0.5;
                    scoreValue = 1.2;
                    resistance = 7;
                ...
                case 13: // Barrier-Placing Square
                    SetStats(Color.Brown, 800, 7, 3, 30, new int[] { 4, 11, 7 }, new int[] { 4, 2, 1 });
                    reload = 120;
                    break;
                case 14: // Helper Triangle
                    SetStats(Color.Blue, 1200, 10, 20, 20);
                    homing = 1;
                    weight = 0.5;
                    break;
                case 15: // Small Laser-Shooting Heptagon
                    SetStats(Color.LightSkyBlue, 700, 4, 2, 14, new int[] { 1, 5 }, new int[] { 3, 2 });
                    weight = 0.1;
                    targetType = 2;
                    break;
                // missiles
